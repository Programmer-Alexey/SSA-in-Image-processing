...
if (!bm3d_available) {
  stop("Python-модуль 'bm3d' недоступен, а он нужен для главы 3.")
}
...
method_params_df <- data.frame(
  ...
  "Параметры" = c(
    "1 компонента; DFT по строкам, CSSA по строкам, обратное DFT по строкам",
    "1 компонента; DFT по строкам, CSSA по строкам, обратное DFT по столбцам",
    "1 компонента; DFT по строкам, CSSA по столбцам, обратное DFT по строкам",
    "1 компонента; DFT по строкам, CSSA по столбцам, обратное DFT по столбцам",
    "Окно 3 x 3",
    "Локальное окно 5 x 5",
    "sigma_psd = 0.2",
    "Порог по квантилю 0.9"
  ),
  check.names = FALSE
)
