...
load_existing_parameter_samples <- function(sigma_values) {
  known_files <- data.frame(
    sigma = c(0.2, 0.05),
    file = c(
      "line_experiment3_ht_error_samples.csv",
...
## ----run-experiment-----------------------------------------------------------
samples_csv <- "line_experiment_rt_distance_samples.csv"
...
## ----save-tables--------------------------------------------------------------
slides_tables_dir <- file.path("VKR", "VKR_SLIDES", "SSA Slides", "tables")
...
write.csv(
  distance_summary,
  "line_experiment_rt_distance_summary.csv",
  row.names = FALSE
)
...
saveRDS(
  ...
  "line_experiment_rt_distance_results.rds"
)
